...
if (isset($_GET['p']) && is_numeric($_GET['p'])) { // Already been determined.
    ...
    $sql = "SELECT COUNT(`USERS_ID`) FROM `USER_later` WHERE USERS_ID=$current_id";
...
<div class="w3-container w3-card w3-white">
  <h2 class="w3-text-grey w3-padding-16">
    <span style="color:goldenrod;" class="glyphicon glyphicon-star"> Later</span> 
  </h2>
  <?php
...
if ($pages > 1) {
    ...
    if ($current_page != 1) {
        echo '<a href="later.php?id=' . $current_id . '&s=' . ($start - $display) . '&p=' . $pages . '">Previous</a> ';
    }
    // Make all the numbered pages:
    for ($i = 1;$i <= $pages;$i++) {
        if ($i != $current_page) {
            echo '<a href="later.php?id=' . $current_id . '&s=' . (($display * ($i - 1))) . '&p=' . $pages . '">' . $i . '</a> ';
        } else {
            echo $i . ' ';
        }
    } // End of FOR loop.
    ...
    if ($current_page != $pages) {
        echo '<a href="later.php?id=' . $current_id . '&s=' . ($start + $display) . '&p=' . $pages . '">Next</a>';
    }
